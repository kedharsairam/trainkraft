...
* POLITENESS CONTRACT: 2.5 s base delay + up to 0.5 s jitter between calls;
  ...
  transport-side failures anywhere → 15-minute cooldown. Fatal content
  (AlertMsg incl. "No Avg. Delay Record", bad shape, decrypt failure) is
  never retried. Raised 1.2 → 2.0 → 2.5 s on 2026-09-23 night after the
  server answered bursts with connection resets. Run off-peak IST
  (~00:00–05:00); the script enforces nothing about wall-clock time —
  the operator picks the window.
* Wall time ≈ 10 h for ~10.5k trains (~3.2 s/call incl. latency); the
